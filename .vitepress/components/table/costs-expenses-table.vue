...
// 表格标题
const tableCaption = computed(() => "成本费用数据");
...
const tableData = computed(() => {
  const hasQuarter = costsExpensesData.some((v) => v.year.includes("Q"));

  // 获取原始数据并确保有足够的数据点
  const originalData = costsExpensesData.slice(0, hasQuarter ? 11 : 10);
